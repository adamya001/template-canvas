...
import { 
  ...
  CopyPlus, CheckSquare, Compass, Gift, Heart, Star, Mail, MapPin, Smile, Globe, Info, HelpCircle, FileUp
} from "lucide-react";
...
export default function CanvasEditor({
  ...
  const handleBulkGenerate = async (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (!file) return;
    ...
    reader.onload = async (uploadEvent) => {
      try {
        const data = new Uint8Array(uploadEvent.target?.result as ArrayBuffer);
        ...
        const elementStage = document.getElementById("editor-canvas-stage");
        if (!elementStage) {
          setIsBulkGenerating(false);
          return;
        }
        ...
        for (let i = 0; i < jsonData.length; i++) {
          ...
          const mappedElements = originalElements.map((el: CanvasElement) => {
            if (el.type === 'text' && el.text) {
              let newText = el.text;
              Object.keys(row).forEach(key => {
                const placeholder = `{{${key}}}`;
                if (newText.includes(placeholder)) {
                  newText = newText.replace(new RegExp(placeholder, 'g'), String(row[key]));
                }
              });
          ...
          const canvas = await safeHtml2Canvas(elementStage, {
            scale: 2.2,
            useCORS: true,
        ...
        pdf.save(`bulk-${title.toLowerCase().replace(/\s+/g, "-")}.pdf`);
          ...
          {user?.role === 'admin' && (
            <>
              ...
              <button
                ...
                className="px-3.5 py-1.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg text-xs font-semibold transition-all flex items-center gap-1.5 shrink-0"
              >
                {isBulkGenerating ? <Loader2 className="w-4 h-4 animate-spin" /> : <FileUp className="w-4 h-4" />}
                <span>{isBulkGenerating ? "Generating..." : "Bulk Generate"}</span>
              </button>
            </>
          )}
